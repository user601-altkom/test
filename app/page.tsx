...
function formatujNadplate(nadplata: NadplataFormularza): string {
  return `${nadplata.miesiac}:${nadplata.kwota}:${nadplata.tryb}`;
}
...
export default function Strona() {
  ...
  function dodajNadplate() {
    setNadplaty((poprzednie) => [
      ...poprzednie,
      { id: Date.now(), miesiac: '1', kwota: '10000', tryb: 'obniz_rate' },
    ]);
  }
  ...
  function eksportujCsv() {
    if (!wynik) return;
    const naglowek = 'Numer;Data;Kapitał;Odsetki;Rata;Saldo po spłacie';
        ...
        <section className="grid gap-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.4fr)]">
          <form className="surface flex flex-col gap-6 p-5 sm:p-7" onSubmit={oblicz}>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="field sm:col-span-2">Kwota kredytu (zł)<input required inputMode="decimal" value={kwota} onChange={(e) => setKwota(e.target.value)} /></label>
              <label className="field">Liczba rat<input required type="number" min="1" value={liczbaRat} onChange={(e) => setLiczbaRat(e.target.value)} /></label>
              ...
              <label className="field sm:col-span-2">Marża banku (pp)<input required inputMode="decimal" value={marza} onChange={(e) => setMarza(e.target.value)} /></label>
            </div>
            ...
            <div className="border-t border-[var(--kolor-divider)] pt-5">
              ...
              {nadplaty.length === 0 ? <p className="text-sm italic text-[var(--kolor-muted)]">Brak nadpłat. Harmonogram bazowy.</p> : <div className="flex flex-col gap-3">{nadplaty.map((nadplata) => <div className="grid gap-2 border border-[var(--kolor-divider)] p-3 sm:grid-cols-[0.7fr_1fr_1.4fr_auto]" key={nadplata.id}><label className="field">Rata<input min="1" type="number" value={nadplata.miesiac} onChange={(e) => aktualizujNadplate(nadplata.id, 'miesiac', e.target.value)} /></label><label className="field">Kwota (zł)<input inputMode="decimal" value={nadplata.kwota} onChange={(e) => aktualizujNadplate(nadplata.id, 'kwota', e.target.value)} /></label><label className="field">Tryb<select value={nadplata.tryb} onChange={(e) => aktualizujNadplate(nadplata.id, 'tryb', e.target.value)}><option value="obniz_rate">Obniż ratę</option><option value="skroc_okres">Skróć okres</option></select></label><button type="button" className="remove-button" aria-label="Usuń nadpłatę" onClick={() => setNadplaty((poprzednie) => poprzednie.filter((element) => element.id !== nadplata.id))}>×</button></div>)}</div>}
            </div>
